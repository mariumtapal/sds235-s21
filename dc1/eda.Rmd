...
list_of_files <-
  list.files(
    path = "~/Documents/Spring 2021/SDS 235/sds235/dc1/data/News Articles",
    recursive = TRUE,
    pattern = "\\.txt$",
    full.names = TRUE
  )
...
head <- sums[1:75,]
wordcloud(words = head$term, freq = head$count, min.freq = 2,
...
Very negative sentiments in Alderwood - something's wrong
...
ggplot(sent3, aes(x = emotion, y = count, fill = emotion)) + geom_bar(stat = "identity") + theme_minimal() + theme(legend.position="none", panel.grid.major = element_blank()) + labs( x = "Emotion", y = "Total Count") + ggtitle("Sentiment of Alderwood News Artclies") + theme(plot.title = element_text(hjust=0.5))
```
